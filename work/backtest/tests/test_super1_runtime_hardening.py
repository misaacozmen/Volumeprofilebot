...
def _config() -> dict[str, object]:
    # Runtime account material is intentionally external. This suite uses the
    # tracked structural fixture, never a local operator config.
    return json.loads((ROOT / "tests/fixtures/super1_xm_mt5_demo_config.json").read_text(encoding="utf-8"))
...
$used = @()
foreach ($file in @(Get-ChildItem -LiteralPath '{root_literal}\\deploy' -Filter '*.ps1' -File | Where-Object {{
    $_.Name -notin @('build_signed_windows_release.ps1', 'release_integrity.ps1')
}})) {{
    $tokens = $null
    $parseErrors = $null
