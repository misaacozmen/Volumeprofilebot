...
def _same_sqlite_process_worker(db_path: str, approval_id: str, name: str, barrier, result_queue) -> None:
    ...
    try:
        ...
        # The production guard requires a strictly newer read boundary.  Build
        # the second snapshot from the first boundary so this process test is
        # deterministic on Python versions with different clock resolution.
        second = _fresh_second(first)
