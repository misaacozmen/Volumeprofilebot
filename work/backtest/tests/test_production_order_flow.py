...
def test_actual_super1_production_flow_sends_once_then_reconciles_fake_mt5(tmp_path: Path) -> None:
    ...
    snapshots = iter(
        (
                BrokerSnapshot(NOW, 10_000.0, 5_000.0, daily_realized_r=0.0, strategy_health="ACTIVE", halt=False, instrument_contract=contract, total_stop_risk=0.0, snapshot_hash="1" * 64, policy_hash=policy.policy_hash, instrument_contract_hash=contract.contract_hash, order_calc_profit=profit, order_calc_margin=margin, total_entry_count=0, entry_counts_by_instrument={"i1": 0}, open_position_counts_by_instrument={"i1": 0}, pending_order_counts_by_instrument={"i1": 0}, leverage=2.0, pair_exposure_percent=0.0, concentration_percent=0.0, deal_facts_hash="d" * 64, deal_reconciliation_at=NOW, deal_watermark_time_msc=0, deal_watermark_ticket=0, account_login=account_key, broker_query_id="q1"),
                BrokerSnapshot(NOW, 10_000.0, 5_000.0, daily_realized_r=0.0, strategy_health="ACTIVE", approval=True, halt=False, instrument_contract=contract, total_stop_risk=0.0, snapshot_hash="2" * 64, policy_hash=policy.policy_hash, instrument_contract_hash=contract.contract_hash, order_calc_profit=profit, order_calc_margin=margin, total_entry_count=0, entry_counts_by_instrument={"i1": 0}, open_position_counts_by_instrument={"i1": 0}, pending_order_counts_by_instrument={"i1": 0}, leverage=2.0, pair_exposure_percent=0.0, concentration_percent=0.0, deal_facts_hash="d" * 64, deal_reconciliation_at=NOW, deal_watermark_time_msc=0, deal_watermark_ticket=0, account_login=account_key, broker_query_id="q2"),
        )
    )
