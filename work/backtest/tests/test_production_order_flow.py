...
def test_actual_super1_production_flow_sends_once_then_reconciles_fake_mt5(tmp_path: Path) -> None:
    proposal = _proposal()
    campaign_id, account_key, release_id = "campaign-1", "740000001", "release-1"
    ...
    snapshots = iter(
        (
            BrokerSnapshot(NOW, 10_000.0, 5_000.0, daily_realized_r=0.0, strategy_health="ACTIVE", halt=False, instrument_contract=contract, total_stop_risk=0.0, snapshot_hash="1" * 64, policy_hash="3" * 64, order_calc_profit=profit, order_calc_margin=margin, account_login=account_key, broker_query_id="q1", query_started_at=NOW - timedelta(seconds=3), query_completed_at=NOW - timedelta(seconds=2), broker_query_sequence=1, broker_read_operations=("account_info", "positions_get", "orders_get", "history_deals_get")),
            BrokerSnapshot(NOW, 10_000.0, 5_000.0, daily_realized_r=0.0, strategy_health="ACTIVE", approval=True, halt=False, instrument_contract=contract, total_stop_risk=0.0, snapshot_hash="2" * 64, policy_hash="3" * 64, order_calc_profit=profit, order_calc_margin=margin, account_login=account_key, broker_query_id="q2", query_started_at=NOW + timedelta(seconds=1), query_completed_at=NOW + timedelta(seconds=2), broker_query_sequence=2, broker_read_operations=("account_info", "positions_get", "orders_get", "history_deals_get")),
        )
    )
