...
DEAL_TIME = int((NOW - timedelta(minutes=1)).timestamp())
...
def test_broker_facts_derive_realized_r_from_partial_position_exits_and_costs() -> None:
    snapshot = _snapshot(_builder(deals=(
        {"deal_id": 1, "position_id": "p-1", "entry": 1, "magic": 7, "time": DEAL_TIME, "profit": 50.0, "commission": -2.0, "swap": -1.0, "fee": 0.0},
        {"deal_id": 2, "position_id": "p-1", "entry": "OUT", "magic": 7, "time": DEAL_TIME, "profit": 25.0, "commission": -1.0, "swap": 0.0, "fee": -0.5},
    )))
...
def test_daily_loss_cap_is_evaluated_from_broker_facts() -> None:
    snapshot = _snapshot(_builder(deals=(
        {"deal_id": 1, "position_id": "p-1", "entry": 1, "magic": 7, "time": DEAL_TIME, "profit": -80.0, "commission": -15.0, "swap": -10.0, "fee": -1.0},
    )))
...
def test_sl_less_exposure_and_nan_fail_closed() -> None:
    ...
    with pytest.raises(BrokerFactsError, match="commission"):
        _snapshot(_builder(deals=({"deal_id": 1, "position_id": "p-1", "entry": 1, "magic": 7, "time": DEAL_TIME, "profit": 1.0, "swap": 0.0, "fee": 0.0},)))
    with pytest.raises(BrokerFactsError, match="duplicate"):
        _snapshot(_builder(deals=(
            {"deal_id": 1, "position_id": "p-1", "entry": 1, "magic": 7, "time": DEAL_TIME, "profit": 1.0, "commission": 0.0, "swap": 0.0, "fee": 0.0},
            {"deal_id": 1, "position_id": "p-1", "entry": 1, "magic": 7, "time": DEAL_TIME, "profit": 1.0, "commission": 0.0, "swap": 0.0, "fee": 0.0},
        )))
...
def test_daily_deal_query_starts_at_new_york_trading_day_not_caller_history_window() -> None:
    ...
    assert observed == [(datetime(2026, 8, 1, tzinfo=timezone.utc), NOW)]
