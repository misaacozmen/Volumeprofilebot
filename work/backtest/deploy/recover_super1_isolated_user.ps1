...
$IdentityHelper = Join-Path $App "deploy\broker_identity_env.ps1"
if (-not (Test-Path -LiteralPath $IdentityHelper -PathType Leaf)) { throw "Broker identity helper is missing: $IdentityHelper" }
. $IdentityHelper
...
$ErrorActionPreference = "Stop"
$Root = "C:\Super1"
$IdentityHelper = Join-Path $Root "app\deploy\broker_identity_env.ps1"
if (-not (Test-Path -LiteralPath $IdentityHelper -PathType Leaf)) { throw "Broker identity helper is missing." }
. $IdentityHelper
$ExpectedLogin = Get-BrokerIdentityLogin
...
try {
    ...
    if ($Discovery.state -ne "FOUND" -or -not $Discovery.demo_verified -or
        [long]$Discovery.login -ne [long]$ExpectedLogin -or
...
'@
$WorkerSource = $WorkerSource.Replace("C:\Super1", $Root).Replace("%SUPER1_PYTHON%", $Python).Replace("%SUPER1_APP%", $App)
...
try {
    ...
    $StartInfo.EnvironmentVariables["XM_MT5_ACCOUNT_LOGIN"] = [string]$ExpectedLogin
