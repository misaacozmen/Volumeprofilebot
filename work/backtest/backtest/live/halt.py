...
class HaltController:
    ...
    @contextmanager
    def _flatten_claim_lock(self):
        self.flatten_attempt_root.mkdir(parents=True, exist_ok=True)
        # SQLite's RESERVED transaction lock is process-safe on Windows and
        # POSIX, and is released by the OS if a claimant dies.  Keep claim
        # read/stale-check/write and token-checked release in one transaction.
        lock_path = self.flatten_attempt_root / ".claims.lock.sqlite3"
    ...
    @staticmethod
    def _claim_payload(path: Path) -> Mapping[str, Any]:
        try:
            value = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            raise HaltError("flatten claim cannot be read") from exc
        if not isinstance(value, Mapping):
            raise HaltError("flatten claim is malformed")
        return value

    def claim_flatten_cycle(self, episode_id: str | None = None) -> str | None:
        episode = episode_id or self.episode_id()
        claim = self.flatten_attempt_root / f"episode.{episode}.cycle.claim.json"
        owner = f"{os.getpid()}:{threading.get_ident()}"
        owner_token = uuid4().hex
        with self._flatten_claim_lock():
            if claim.exists():
                existing = self._claim_payload(claim)
    ...
    def release_flatten_cycle(self, episode_id: str | None = None, *, owner_token: str) -> None:
        episode = episode_id or self.episode_id()
        claim = self.flatten_attempt_root / f"episode.{episode}.cycle.claim.json"
        with self._flatten_claim_lock():
            if not claim.exists():
                return
            existing = self._claim_payload(claim)
            if str(existing.get("owner_token") or "") == owner_token:
                claim.unlink()

    def claim_flatten(self, episode_id: str | None = None, *, ticket: int | str | None = None) -> str | None:
        episode = episode_id or self.episode_id()
    ...
    def release_claim(self, episode_id: str | None = None, *, ticket: int | str | None = None, owner_token: str) -> None:
        episode = episode_id or self.episode_id()
        key = str(ticket or "episode")
        claim = self.flatten_attempt_root / f"episode.{episode}.{key}.claim.json"
        with self._flatten_claim_lock():
    ...
    def mark_side_effect_attempted(self, episode_id: str | None = None, *, ticket: int | str | None = None, owner_token: str) -> None:
        episode = episode_id or self.episode_id()
        key = str(ticket or "episode")
        if not key.isalnum() and key != "episode":
            raise HaltError("flatten ticket key is invalid")
        ...
        with self._flatten_claim_lock():
            ...
            if str(claim_payload.get("owner_token") or "") != owner_token:
                raise HaltError("flatten claim owner token mismatch")
            history = list(claim_payload.get("state_history", ["DISCOVERED", "CLAIMED"]))
            ...
            updated_claim = dict(claim_payload)
            updated_claim.update({"state": "WRITE_ATTEMPTED", "state_history": history, "side_effect_attempted": True})
            _atomic_json(claim, updated_claim)
...
def _emergency_flatten_cycle_locked(
    controller: HaltController,
    ...
    try:
        ...
        for action in actions:
            ...
            ticket_token = controller.claim_flatten(episode, ticket=ticket)
            if ticket_token is None:
                continue
            attempted += 1
            try:
                controller.mark_side_effect_attempted(episode, ticket=ticket, owner_token=ticket_token)
                ...
                controller.release_claim(episode, ticket=ticket, owner_token=ticket_token)
        if attempted == 0:
...
def emergency_flatten_cycle(
    ...
    """Serialize one flatten cycle while preserving restart recovery."""
    episode = controller.episode_id()
    cycle_token = controller.claim_flatten_cycle(episode)
    if cycle_token is None:
        return {"state": "FLATTEN_LATCHED", "halt_episode_id": episode}
    try:
        return _emergency_flatten_cycle_locked(controller, read_exposure, cancel_or_close)
    finally:
        controller.release_flatten_cycle(episode, owner_token=cycle_token)
