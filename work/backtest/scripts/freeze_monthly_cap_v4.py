...
REPORT = ROOT / "outputs" / "reports" / "strategy_improvement_loop_v4" / "iteration_075" / "locked_monthly_cap_v1"
CANDIDATE_DIR = ROOT / "research_candidates" / "v13_strategy_loop"
CAP_R = 9.0


def apply(frame: pd.DataFrame) -> pd.DataFrame:
    return causal_period_loss_cap(frame, CAP_R, "month")
...
def main() -> None:
    ...
    payload = {
        "name": "NQ_SPX_LOCKED_PAIR_MONTHLY_CAP_V1", "status": "LOCAL_LOCKED_PENDING_HISTORICAL_AUDIT",
        "base_candidate": str(BASE), "monthly_realized_loss_cap_r": CAP_R, "reset": "calendar_month",
        ...
        "deterministic": stable_hash(selected) == stable_hash(repeated), "graph_evidence_pass": graph_pass,
    ...
    candidate_path = CANDIDATE_DIR / "nq_spx_locked_pair_monthly_cap_v1.json"
