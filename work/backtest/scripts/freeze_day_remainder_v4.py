...
from run_strategy_improvement_loop_v4 import causal_loss_day_cooldown  # noqa: E402
...
REPORT = ROOT / "outputs" / "reports" / "strategy_improvement_loop_v4" / "iteration_086" / "locked_day_remainder_v1"
CANDIDATE_DIR = ROOT / "research_candidates" / "v16_strategy_loop"


def apply(frame: pd.DataFrame) -> pd.DataFrame:
    return causal_loss_day_cooldown(frame, 0)
...
def main() -> None:
    ...
    payload = {
        "name": "NQ_SPX_LOCKED_PAIR_DAY_REMAINDER_V1", "status": "LOCAL_LOCKED_PENDING_HISTORICAL_AUDIT",
        "base_candidate": str(BASE), "rule": "after any terminal loss, skip entries for remainder of UTC calendar day",
        "state_rule": "loss exit_time <= current entry_time", "open_trade_outcome_used": False,
    ...
    candidate_path = CANDIDATE_DIR / "nq_spx_locked_pair_day_remainder_v1.json"
