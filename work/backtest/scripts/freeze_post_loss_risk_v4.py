...
from run_strategy_improvement_loop_v4 import causal_post_loss_scale  # noqa: E402
...
REPORT = ROOT / "outputs" / "reports" / "strategy_improvement_loop_v4" / "iteration_091" / "locked_post_loss_risk_v1"
CANDIDATE_DIR = ROOT / "research_candidates" / "v17_strategy_loop"
PARAMETERS = {"scaled_signals": 1, "loss_scale": 0.75, "healthy_scale": 1.24}


def apply(frame: pd.DataFrame) -> pd.DataFrame:
    return causal_post_loss_scale(frame, **PARAMETERS)
...
def main() -> None:
    ...
    payload = {
        "name": "NQ_SPX_LOCKED_PAIR_POST_LOSS_RISK_V1", "status": "LOCAL_LOCKED_PENDING_HISTORICAL_AUDIT",
        "base_candidate": str(BASE), "risk_rule": PARAMETERS,
        "state_rule": "terminal loss scales next signal; open trades excluded", "open_trade_outcome_used": False,
        ...
        "result_sha256": result_hash(selected), "code_sha256": sha256(Path(__file__).read_bytes()).hexdigest(), "config_sha256": "",
    }
    payload["config_sha256"] = sha256(json.dumps({k: v for k, v in payload.items() if k != "config_sha256"}, sort_keys=True).encode()).hexdigest()
    candidate_path = CANDIDATE_DIR / "nq_spx_locked_pair_post_loss_risk_v1.json"
