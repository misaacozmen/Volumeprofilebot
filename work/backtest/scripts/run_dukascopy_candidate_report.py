from __future__ import annotations

import csv
import html
import subprocess
import sys
from dataclasses import dataclass
from pathlib import Path

import pandas as pd


ROOT = Path(__file__).resolve().parents[1]
RAW_DIR = ROOT / "data" / "raw"
REPORT_DIR = ROOT / "outputs" / "reports" / "dukascopy_candidate_full_report"
REWARD_LEVELS = [1.0, 2.0, 3.0, 4.0]


@dataclass(frozen=True)
class Candidate:
    slug: str
    name: str
    symbol: str
    timeframe: str
    max_trades_per_day: int
    weekdays: str
    setup_type: str = "all"
    direction: str = "all"


CANDIDATES = [
    Candidate(
        slug="nq_3m_max1_no_tuesday",
        name="NQ - 3m - Tuesday hariç - max 1 trade",
        symbol="DUKASCOPY_USATECHIDXUSD",
        timeframe="3m",
        max_trades_per_day=1,
        weekdays="Monday,Wednesday,Thursday,Friday",
    ),
    Candidate(
        slug="spx_5m_max2_tue_wed_fri_fvg",
        name="SPX - 5m - Tue/Wed/Fri - FVG - max 2 trade",
        symbol="DUKASCOPY_USA500IDXUSD",
        timeframe="5m",
        max_trades_per_day=2,
        weekdays="Tuesday,Wednesday,Friday",
        setup_type="fvg",
    ),
    Candidate(
        slug="silver_5m_max1_tue_wed_thu",
        name="Silver - 5m - Tue/Wed/Thu - max 1 trade",
        symbol="DUKASCOPY_XAGUSD",
        timeframe="5m",
        max_trades_per_day=1,
        weekdays="Tuesday,Wednesday,Thursday",
    ),
]


def main() -> None:
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    rows: list[dict[str, object]] = []
    all_trades: list[pd.DataFrame] = []

    for candidate in CANDIDATES:
        for reward_r in REWARD_LEVELS:
            out_dir = REPORT_DIR / candidate.slug / f"rr_{int(reward_r)}"
            run_backtest(candidate, reward_r, out_dir)
            summary = pd.read_csv(out_dir / "summary.csv").iloc[0].to_dict()
            trades = pd.read_csv(out_dir / "trades.csv")
            monthly = pd.read_csv(out_dir / "monthly_stats.csv")
            row = build_summary_row(candidate, reward_r, out_dir, summary, trades, monthly)
            rows.append(row)
            if not trades.empty:
                trades.insert(0, "reward_r", reward_r)
                trades.insert(0, "candidate", candidate.name)
                trades.insert(0, "candidate_slug", candidate.slug)
                all_trades.append(trades)

    comparison = pd.DataFrame(rows)
    comparison.to_csv(REPORT_DIR / "comparison.csv", index=False)
    if all_trades:
        pd.concat(all_trades, ignore_index=True).to_csv(REPORT_DIR / "all_trades.csv", index=False)

    write_markdown_report(comparison)
    write_html_report(comparison)
    print(f"Wrote report folder: {REPORT_DIR}")
    print(f"HTML: {REPORT_DIR / 'report.html'}")
    print(f"Markdown: {REPORT_DIR / 'report.md'}")


def run_backtest(candidate: Candidate, reward_r: float, out_dir: Path) -> None:
    if (out_dir / "summary.csv").exists() and (out_dir / "trades.csv").exists():
        print(f"Skipping existing {candidate.slug} {reward_r:.0f}R")
        return
    cmd = [
        sys.executable,
        "-m",
        "backtest.cli",
        "run",
        str(RAW_DIR),
        "--symbol",
        candidate.symbol,
        "--timeframe",
        candidate.timeframe,
        "--max-trades-per-day",
        str(candidate.max_trades_per_day),
        "--allowed-weekdays",
        candidate.weekdays,
        "--setup-type",
        candidate.setup_type,
        "--direction",
        candidate.direction,
        "--entry-mode",
        "start",
        "--reward-r",
        str(reward_r),
        "--stop-management",
        "none",
        "--output-dir",
        str(out_dir),
    ]
    print(f"Running {candidate.slug} {reward_r:.0f}R")
    subprocess.run(cmd, cwd=ROOT, check=True)


def build_summary_row(
    candidate: Candidate,
    reward_r: float,
    out_dir: Path,
    summary: dict[str, object],
    trades: pd.DataFrame,
    monthly: pd.DataFrame,
) -> dict[str, object]:
    max_dd = float(summary.get("max_drawdown_r", 0) or 0)
    net_r = float(summary.get("net_r", 0) or 0)
    net_r_per_dd = round(net_r / abs(max_dd), 2) if max_dd else ""
    years = yearly_stats(trades)
    streaks = streak_stats(trades)
    return {
        "candidate": candidate.name,
        "candidate_slug": candidate.slug,
        "reward_r": reward_r,
        "symbol": candidate.symbol,
        "timeframe": candidate.timeframe,
        "weekdays": candidate.weekdays,
        "setup_type": candidate.setup_type,
        "max_trades_per_day": candidate.max_trades_per_day,
        "trades": int(summary.get("total_trades", 0) or 0),
        "wins": int(summary.get("wins", 0) or 0),
        "losses": int(summary.get("losses", 0) or 0),
        "open_trades": int(summary.get("open_trades", 0) or 0),
        "win_rate": float(summary.get("win_rate", 0) or 0),
        "net_r": net_r,
        "avg_r": float(summary.get("avg_r", 0) or 0),
        "max_drawdown_r": max_dd,
        "net_r_per_dd": net_r_per_dd,
        "profit_factor": float(summary.get("profit_factor", 0) or 0),
        "best_day": summary.get("best_day", ""),
        "worst_day": summary.get("worst_day", ""),
        "long_stats": summary.get("long_stats", ""),
        "short_stats": summary.get("short_stats", ""),
        "best_month": best_period(monthly, "month"),
        "worst_month": worst_period(monthly, "month"),
        "positive_months": positive_count(monthly),
        "negative_months": negative_count(monthly),
        "yearly_net_r": years,
        "max_win_streak": streaks["max_win_streak"],
        "max_loss_streak": streaks["max_loss_streak"],
        "out_dir": str(out_dir.relative_to(ROOT)),
    }


def yearly_stats(trades: pd.DataFrame) -> str:
    if trades.empty:
        return ""
    frame = trades.copy()
    frame["year"] = pd.to_datetime(frame["date"]).dt.year
    grouped = frame.groupby("year")["r_multiple"].sum().round(2)
    return "; ".join(f"{year}:{value:.2f}R" for year, value in grouped.items())


def streak_stats(trades: pd.DataFrame) -> dict[str, int]:
    max_win = max_loss = cur_win = cur_loss = 0
    if trades.empty:
        return {"max_win_streak": 0, "max_loss_streak": 0}
    for result in trades["result"].astype(str):
        if result == "win":
            cur_win += 1
            cur_loss = 0
        elif result == "loss":
            cur_loss += 1
            cur_win = 0
        else:
            cur_win = 0
            cur_loss = 0
        max_win = max(max_win, cur_win)
        max_loss = max(max_loss, cur_loss)
    return {"max_win_streak": max_win, "max_loss_streak": max_loss}


def best_period(monthly: pd.DataFrame, column: str) -> str:
    if monthly.empty:
        return ""
    row = monthly.loc[monthly["net_r"].idxmax()]
    return f"{row[column]}:{float(row['net_r']):.2f}R"


def worst_period(monthly: pd.DataFrame, column: str) -> str:
    if monthly.empty:
        return ""
    row = monthly.loc[monthly["net_r"].idxmin()]
    return f"{row[column]}:{float(row['net_r']):.2f}R"


def positive_count(monthly: pd.DataFrame) -> int:
    return int((monthly["net_r"] > 0).sum()) if not monthly.empty else 0


def negative_count(monthly: pd.DataFrame) -> int:
    return int((monthly["net_r"] < 0).sum()) if not monthly.empty else 0


def best_by_balance(group: pd.DataFrame) -> pd.Series:
    ranked = group.copy()
    ranked["balance_score"] = (
        ranked["net_r"].astype(float)
        + ranked["net_r_per_dd"].replace("", 0).astype(float) * 3
        + ranked["profit_factor"].astype(float) * 5
    )
    return ranked.sort_values(["balance_score", "net_r"], ascending=False).iloc[0]


def best_rows_by_candidate(comparison: pd.DataFrame) -> pd.DataFrame:
    selected = comparison.groupby("candidate", group_keys=False).apply(best_by_balance)
    if "candidate" not in selected.columns:
        selected = selected.reset_index()
    return selected.reset_index(drop=True)


def write_markdown_report(comparison: pd.DataFrame) -> None:
    selected = best_rows_by_candidate(comparison)
    best_columns = [
        "candidate",
        "reward_r",
        "trades",
        "win_rate",
        "net_r",
        "max_drawdown_r",
        "net_r_per_dd",
        "profit_factor",
        "best_month",
        "worst_month",
    ]
    grid_columns = [
        "candidate",
        "reward_r",
        "trades",
        "win_rate",
        "net_r",
        "avg_r",
        "max_drawdown_r",
        "net_r_per_dd",
        "profit_factor",
        "positive_months",
        "negative_months",
        "max_loss_streak",
    ]
    lines = [
        "# Dukascopy Full Data Candidate Report",
        "",
        "Data range: 2022-01-02 18:00 NY -> 2026-07-02 23:55/23:57 NY",
        "Entry model: FVG/IFVG start. Stop management: none.",
        "",
        "## Best Balance Per Candidate",
        "",
        markdown_table(selected[best_columns]),
        "",
        "## Full RR Grid",
        "",
        markdown_table(comparison[grid_columns]),
        "",
    ]
    for _, row in selected.iterrows():
        lines.extend(candidate_detail_block(row))
    (REPORT_DIR / "report.md").write_text("\n".join(lines), encoding="utf-8")


def markdown_table(frame: pd.DataFrame) -> str:
    headers = [str(column) for column in frame.columns]
    lines = [
        "| " + " | ".join(headers) + " |",
        "| " + " | ".join("---" for _ in headers) + " |",
    ]
    for _, row in frame.iterrows():
        values = [markdown_cell(row[column]) for column in frame.columns]
        lines.append("| " + " | ".join(values) + " |")
    return "\n".join(lines)


def markdown_cell(value: object) -> str:
    if pd.isna(value):
        return ""
    if isinstance(value, float):
        value = f"{value:.2f}".rstrip("0").rstrip(".")
    return str(value).replace("|", "\\|")


def candidate_detail_block(row: pd.Series) -> list[str]:
    return [
        f"## {row['candidate']}",
        "",
        f"- Best balance RR: {row['reward_r']:.0f}R",
        f"- Net R / DD: {row['net_r']:.2f}R / {row['max_drawdown_r']:.2f}R",
        f"- PF: {row['profit_factor']:.2f}",
        f"- Yearly net R: {row['yearly_net_r']}",
        f"- Best month: {row['best_month']}",
        f"- Worst month: {row['worst_month']}",
        f"- Max loss streak: {row['max_loss_streak']}",
        f"- Output: `{row['out_dir']}`",
        "",
    ]


def write_html_report(comparison: pd.DataFrame) -> None:
    selected = best_rows_by_candidate(comparison)
    cards = "\n".join(summary_card(row) for _, row in selected.iterrows())
    rr_tables = "\n".join(candidate_rr_table(name, group) for name, group in comparison.groupby("candidate"))
    selected_details = "\n".join(candidate_html_detail(row) for _, row in selected.iterrows())
    html_doc = f"""<!doctype html>
<html lang="tr">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Dukascopy Candidate Full Report</title>
  <style>
    :root {{
      --bg: #f7f8fa;
      --panel: #ffffff;
      --ink: #17202a;
      --muted: #627084;
      --line: #dfe5ec;
      --good: #117a55;
      --bad: #b42318;
      --accent: #1f6feb;
      --soft: #eef5ff;
    }}
    body {{
      margin: 0;
      background: var(--bg);
      color: var(--ink);
      font: 14px/1.5 "Segoe UI", Arial, sans-serif;
    }}
    header {{
      background: #111827;
      color: white;
      padding: 28px 36px;
    }}
    header h1 {{ margin: 0 0 8px; font-size: 28px; letter-spacing: 0; }}
    header p {{ margin: 0; color: #cbd5e1; }}
    main {{ max-width: 1220px; margin: 0 auto; padding: 28px 24px 48px; }}
    h2 {{ margin: 30px 0 12px; font-size: 20px; }}
    h3 {{ margin: 0 0 10px; font-size: 16px; }}
    .grid {{ display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 14px; }}
    .card, .section {{
      background: var(--panel);
      border: 1px solid var(--line);
      border-radius: 8px;
      padding: 16px;
      box-shadow: 0 1px 2px rgba(16, 24, 40, .04);
    }}
    .muted {{ color: var(--muted); }}
    .metric-row {{ display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; margin-top: 12px; }}
    .metric {{ background: #f8fafc; border: 1px solid var(--line); border-radius: 6px; padding: 9px; }}
    .metric b {{ display: block; font-size: 18px; }}
    .tag {{ display: inline-block; padding: 3px 8px; border-radius: 999px; background: var(--soft); color: var(--accent); font-weight: 600; }}
    table {{ width: 100%; border-collapse: collapse; background: var(--panel); }}
    th, td {{ padding: 9px 10px; border-bottom: 1px solid var(--line); text-align: right; white-space: nowrap; }}
    th:first-child, td:first-child {{ text-align: left; white-space: normal; }}
    th {{ color: var(--muted); font-size: 12px; text-transform: uppercase; letter-spacing: .04em; background: #f8fafc; }}
    .pos {{ color: var(--good); font-weight: 700; }}
    .neg {{ color: var(--bad); font-weight: 700; }}
    .detail-grid {{ display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }}
    .kv {{ display: grid; grid-template-columns: 150px 1fr; gap: 8px; padding: 6px 0; border-bottom: 1px solid var(--line); }}
    .svg-wrap {{ margin-top: 10px; background: #fbfdff; border: 1px solid var(--line); border-radius: 6px; padding: 8px; }}
    @media (max-width: 900px) {{
      .grid, .detail-grid {{ grid-template-columns: 1fr; }}
      .metric-row {{ grid-template-columns: repeat(2, 1fr); }}
      main {{ padding: 20px 12px 36px; }}
      header {{ padding: 22px 18px; }}
    }}
  </style>
</head>
<body>
  <header>
    <h1>Dukascopy Full Data Candidate Report</h1>
    <p>2022-01-02 18:00 NY -> 2026-07-02 23:55/23:57 NY | Entry: FVG/IFVG start | Stop management: none</p>
  </header>
  <main>
    <h2>Best Balance Per Candidate</h2>
    <div class="grid">{cards}</div>
    <h2>RR Grid</h2>
    {rr_tables}
    <h2>Selected Candidate Details</h2>
    <div class="detail-grid">{selected_details}</div>
  </main>
</body>
</html>"""
    (REPORT_DIR / "report.html").write_text(html_doc, encoding="utf-8")


def summary_card(row: pd.Series) -> str:
    return f"""
    <section class="card">
      <h3>{esc(row['candidate'])}</h3>
      <span class="tag">{float(row['reward_r']):.0f}R best balance</span>
      <div class="metric-row">
        <div class="metric"><span class="muted">Net R</span><b class="{klass(row['net_r'])}">{float(row['net_r']):.1f}</b></div>
        <div class="metric"><span class="muted">DD</span><b>{float(row['max_drawdown_r']):.1f}</b></div>
        <div class="metric"><span class="muted">R/DD</span><b>{row['net_r_per_dd']}</b></div>
        <div class="metric"><span class="muted">PF</span><b>{float(row['profit_factor']):.2f}</b></div>
      </div>
      <p class="muted">Trades: {int(row['trades'])} | WR: {float(row['win_rate']):.2f}% | Max loss streak: {int(row['max_loss_streak'])}</p>
    </section>"""


def candidate_rr_table(name: str, group: pd.DataFrame) -> str:
    rows = []
    for _, row in group.sort_values("reward_r").iterrows():
        rows.append(
            "<tr>"
            f"<td>{float(row['reward_r']):.0f}R</td>"
            f"<td>{int(row['trades'])}</td>"
            f"<td>{float(row['win_rate']):.2f}%</td>"
            f"<td class='{klass(row['net_r'])}'>{float(row['net_r']):.1f}</td>"
            f"<td>{float(row['avg_r']):.2f}</td>"
            f"<td>{float(row['max_drawdown_r']):.1f}</td>"
            f"<td>{row['net_r_per_dd']}</td>"
            f"<td>{float(row['profit_factor']):.2f}</td>"
            f"<td>{int(row['positive_months'])}/{int(row['negative_months'])}</td>"
            f"<td>{int(row['max_loss_streak'])}</td>"
            "</tr>"
        )
    return f"""
    <section class="section">
      <h3>{esc(name)}</h3>
      <table>
        <thead><tr><th>RR</th><th>Trades</th><th>WR</th><th>Net R</th><th>Avg R</th><th>DD</th><th>R/DD</th><th>PF</th><th>+/- Months</th><th>Loss Streak</th></tr></thead>
        <tbody>{''.join(rows)}</tbody>
      </table>
    </section>"""


def candidate_html_detail(row: pd.Series) -> str:
    svg = equity_svg(ROOT / str(row["out_dir"]) / "trades.csv")
    return f"""
    <section class="card">
      <h3>{esc(row['candidate'])}</h3>
      <div class="kv"><span class="muted">Selected RR</span><b>{float(row['reward_r']):.0f}R</b></div>
      <div class="kv"><span class="muted">Yearly Net R</span><span>{esc(row['yearly_net_r'])}</span></div>
      <div class="kv"><span class="muted">Best Month</span><span class="pos">{esc(row['best_month'])}</span></div>
      <div class="kv"><span class="muted">Worst Month</span><span class="neg">{esc(row['worst_month'])}</span></div>
      <div class="kv"><span class="muted">Best Day</span><span>{esc(row['best_day'])}</span></div>
      <div class="kv"><span class="muted">Worst Day</span><span>{esc(row['worst_day'])}</span></div>
      <div class="kv"><span class="muted">Long</span><span>{esc(row['long_stats'])}</span></div>
      <div class="kv"><span class="muted">Short</span><span>{esc(row['short_stats'])}</span></div>
      <div class="svg-wrap">{svg}</div>
    </section>"""


def equity_svg(trades_path: Path) -> str:
    if not trades_path.exists():
        return ""
    trades = pd.read_csv(trades_path)
    if trades.empty:
        return ""
    equity = trades["r_multiple"].cumsum()
    values = [0.0] + [float(value) for value in equity]
    width, height, pad = 520, 140, 12
    lo, hi = min(values), max(values)
    if hi == lo:
        hi += 1
        lo -= 1
    points = []
    for index, value in enumerate(values):
        x = pad + index * (width - 2 * pad) / max(1, len(values) - 1)
        y = height - pad - (value - lo) * (height - 2 * pad) / (hi - lo)
        points.append(f"{x:.1f},{y:.1f}")
    color = "#117a55" if values[-1] >= 0 else "#b42318"
    return (
        f'<svg viewBox="0 0 {width} {height}" width="100%" height="140" role="img">'
        f'<line x1="{pad}" y1="{height-pad}" x2="{width-pad}" y2="{height-pad}" stroke="#dfe5ec"/>'
        f'<polyline fill="none" stroke="{color}" stroke-width="2.5" points="{" ".join(points)}"/>'
        f'<text x="{pad}" y="16" fill="#627084" font-size="12">Equity: {values[-1]:.1f}R</text>'
        f"</svg>"
    )


def klass(value: object) -> str:
    return "pos" if float(value) >= 0 else "neg"


def esc(value: object) -> str:
    return html.escape(str(value))


if __name__ == "__main__":
    main()
