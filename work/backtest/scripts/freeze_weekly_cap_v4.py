...
REPORT = ROOT / "outputs" / "reports" / "strategy_improvement_loop_v4" / "iteration_079" / "locked_weekly_cap_v1"
CANDIDATE_DIR = ROOT / "research_candidates" / "v14_strategy_loop"
CAP_R = 5.5


def apply(frame: pd.DataFrame) -> pd.DataFrame:
    return causal_period_loss_cap(frame, CAP_R, "week")


def stable_hash(frame: pd.DataFrame) -> str:
    columns = IDENTITY + ["exit_time", "r_multiple"]
...
def main() -> None:
    ...
    payload = {
        "name": "NQ_SPX_LOCKED_PAIR_WEEKLY_CAP_V1", "status": "LOCAL_LOCKED_PENDING_HISTORICAL_AUDIT",
        "base_candidate": str(BASE), "weekly_realized_loss_cap_r": CAP_R, "reset": "ISO_week",
    ...
    candidate_path = CANDIDATE_DIR / "nq_spx_locked_pair_weekly_cap_v1.json"
